...
void Graph<T>::reduceCost() {
	for (Vertex<T> *vertex : vertexSet) {
	    for(Edge<T> *edge : vertex->getOutgoing()) {
	        edge->cost += edge->orig->dist - edge->dest->dist;
	    }
    }
}
...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    ...
    while(findAugmentationPath(src, snk)) {
        double tmp = 0;
        tmp = findMinResidualAlongPath(src, snk);
        augmentFlowAlongPath(src,snk, tmp);
        dijkstraShortestPath(src);
        reduceCost();
        f += tmp;
    }

    return f > flow ? flow : f;
}
