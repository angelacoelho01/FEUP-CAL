...
bool Graph<T>::addBidirectionalEdge(const T &sourc, const T &dest, double w) {
	//TODO
	Vertex<T> *src = findVertex(sourc), *dst = findVertex(dest);
    if(src == nullptr || dst == nullptr) return false;
	Edge<T> *edge1 = src->addEdge(dst, w);
	Edge<T> *edge2 = dst->addEdge(src, w);
	edge1->reverse = edge2;
	edge2->reverse = edge1;
	return true;
}
...
std::vector<Vertex<T>* > Graph<T>::calculatePrim() {
    ...
    Vertex<T> *origin = priorityQueue.extractMin();
    origin->dist = 0;
    priorityQueue.insert(origin);

    while(!priorityQueue.empty()) {
        Vertex<T> *currentVertex = priorityQueue.extractMin();
        currentVertex->visited = true;
        res.push_back(currentVertex);

        for(Edge<T> *edge : currentVertex->adj) {
            if(edge->getDest()->queueIndex != 0 && edge->getWeight() < edge->getDest()->getDist()) {
                edge->getDest()->dist = edge->getWeight();
                edge->getDest()->path = currentVertex;
            }
        }
    }
    return res;
...
std::vector<Vertex<T>*> Graph<T>::calculateKruskal() {
    ...
    std::sort(edges.begin(), edges.end(), [](const Edge<T>* edge1, const Edge<T>* edge2) {
        return edge1->getWeight() < edge2->getWeight();
    });

    unsigned i = 0;
    while(edgesAccepted < getNumVertex() - 1) {
        Edge<T> *currentEdge = edges[i++]; // currentEdge = (u, v)

        if(currentEdge->selected) continue;

        Vertex<T> *uset = findSet(currentEdge->getOrig());
        Vertex<T> *vset = findSet(currentEdge->getDest());
        if(uset != vset) {
            edgesAccepted++;
            linkSets(uset, vset);
            currentEdge->reverse->selected = true;
...
void Graph<T>::dfsKruskalPath(Vertex<T> *v) {
    ...
    for(Edge<T> *edge : v->adj) {
        if(edge->selected && !edge->getDest()->visited) {
            edge->getDest()->path = v;
